...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$completed && $step === 'configure') {
        ...
        if ($error === '') {
            ...
            try {
                ...
                if ($adminCount === 0) {
                    ...
                    $stmt = $db->prepare('INSERT INTO login (username, password, is_super_admin, status) VALUES (?, ?, 1, "active")');
                    if (!$stmt) {
                ...
                if (!pih_email_ensure_schema($db)) {
                    throw new RuntimeException('Unable to initialize email template schema.');
                }
...
<div class="wrap">
    <div class="card">
        ...
        <div class="body">
            ...
            <?php if ($completed) { ?>
                <div class="actions">
                    <a class="btn btn-primary" href="<?php echo htmlspecialchars($loginUrl, ENT_QUOTES, 'UTF-8'); ?>">Go to Admin Login</a>
                </div>
            <?php } elseif ($step === 'preflight') { ?>
                <div class="preflight">
                    ...
                    <table class="preflight-table">
                        ...
                        <?php foreach ($preflightChecks as $check) { ?>
                            <tr>
                                <td><?php echo htmlspecialchars((string) ($check['label'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></td>
                                <td>
                ...
                <?php if (!$preflightOk && !empty($preflightFixHints)) { ?>
                    <div class="fixbox">
                        ...
                        <?php foreach ($preflightFixHints as $hint) { ?>
                            <div class="fix-item">
                                <div class="fix-title"><?php echo htmlspecialchars((string) ($hint['title'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></div>
                                <p class="fix-note"><?php echo htmlspecialchars((string) ($hint['note'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></p>
                ...
                <div class="actions">
                    <form method="get" action="" style="margin:0;display:inline;">
                        ...
                        <button class="btn btn-primary" type="submit" <?php echo $preflightOk ? '' : 'disabled'; ?>>Next</button>
                    </form>
                    <a class="btn btn-light" href="install.php">Re-check Preflight</a>
                </div>
            <?php } else { ?>
                <p class="hint">Step 2: Complete installation settings. Theme colors, session timeout defaults, and email template defaults are retained from the current database defaults. Live chat is intentionally left empty during installation and can be configured later in admin settings.</p>
                <form method="post" enctype="multipart/form-data" action="?step=configure">
                    ...
                    <div class="grid">
                        <div>
                            ...
                            <input id="site_name" name="site_name" type="text" value="<?php echo htmlspecialchars($fields['site_name'], ENT_QUOTES, 'UTF-8'); ?>" required>
                        </div>
                        <div>
                            <label for="site_short_name">Site Short Name</label>
                            <input id="site_short_name" name="site_short_name" type="text" value="<?php echo htmlspecialchars($fields['site_short_name'], ENT_QUOTES, 'UTF-8'); ?>">
                        </div>
                        <div class="full">
                            <label for="site_addr_primary">Primary Address</label>
                            <textarea id="site_addr_primary" name="site_addr_primary"><?php echo htmlspecialchars($fields['site_addr_primary'], ENT_QUOTES, 'UTF-8'); ?></textarea>
                        </div>
                        <div class="full">
                            <label for="site_addr_secondary">Secondary Address</label>
                            <textarea id="site_addr_secondary" name="site_addr_secondary"><?php echo htmlspecialchars($fields['site_addr_secondary'], ENT_QUOTES, 'UTF-8'); ?></textarea>
                        </div>
                        <div>
                            <label for="site_phone_primary">Primary Phone</label>
                            <input id="site_phone_primary" name="site_phone_primary" type="text" value="<?php echo htmlspecialchars($fields['site_phone_primary'], ENT_QUOTES, 'UTF-8'); ?>">
                        </div>
                        <div>
                            <label for="site_phone_secondary">Secondary Phone</label>
                            <input id="site_phone_secondary" name="site_phone_secondary" type="text" value="<?php echo htmlspecialchars($fields['site_phone_secondary'], ENT_QUOTES, 'UTF-8'); ?>">
                        </div>
                        <div>
                            <label for="site_email_primary">Primary Email</label>
                            <input id="site_email_primary" name="site_email_primary" type="email" value="<?php echo htmlspecialchars($fields['site_email_primary'], ENT_QUOTES, 'UTF-8'); ?>" required>
                        </div>
                        <div>
                            <label for="site_email_secondary">Secondary Email</label>
                            <input id="site_email_secondary" name="site_email_secondary" type="email" value="<?php echo htmlspecialchars($fields['site_email_secondary'], ENT_QUOTES, 'UTF-8'); ?>" required>
                        </div>
                        <div>
                            <label for="site_url">Site URL</label>
                            <input id="site_url" name="site_url" type="url" value="<?php echo htmlspecialchars($fields['site_url'], ENT_QUOTES, 'UTF-8'); ?>" required>
                        </div>
                        <div>
                            <label for="site_host">Site Host</label>
                            <input id="site_host" name="site_host" type="text" value="<?php echo htmlspecialchars($fields['site_host'], ENT_QUOTES, 'UTF-8'); ?>">
                    ...
                    <div class="sec">
                        ...
                        <div class="grid">
                            <div>
                                ...
                                <input id="site_logo" name="site_logo" type="file" accept=".png,.jpg,.jpeg,.gif,.webp,.svg">
                                <div class="small">If omitted, current default logo is copied into media/assets/branding and mapped.</div>
                            </div>
                            <div>
                                <label for="site_favicon">Favicon File (optional upload)</label>
                                <input id="site_favicon" name="site_favicon" type="file" accept=".png,.jpg,.jpeg,.gif,.webp,.svg,.ico">
                                <div class="small">If omitted, current default favicon is copied into media/assets/branding and mapped.</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="sec">
                        <h2>Admin Account</h2>
                        <div class="small">If no admin exists yet, these fields are used to create the first super admin.</div>
                        <div class="grid" style="margin-top:8px;">
                            <div>
                                <label for="admin_username">Admin Username</label>
                                <input id="admin_username" name="admin_username" type="text" value="<?php echo htmlspecialchars($fields['admin_username'], ENT_QUOTES, 'UTF-8'); ?>">
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <input id="admin_password_confirm" name="admin_password_confirm" type="password" autocomplete="new-password">
                            </div>
                        </div>
                    </div>

                    <div class="actions">
                        <button class="btn btn-primary" type="submit" <?php echo $preflightOk ? '' : 'disabled'; ?>>Run Installation</button>
                    </div>
                </form>
            <?php } ?>
        </div>
